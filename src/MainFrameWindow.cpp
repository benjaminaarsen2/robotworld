...
MainFrameWindow::MainFrameWindow(const std::string &aTitle) :
		wxFrame(nullptr, wxID_ANY, aTitle, wxDefaultPosition,
				wxSize(1200, 600)), clientPanel(nullptr), menuBar(nullptr), splitterWindow(
				nullptr), lhsPanel(nullptr), robotWorldCanvas(nullptr), rhsPanel(
				nullptr), logTextCtrl(nullptr), logDestination(nullptr), configPanel(
				nullptr), drawOpenSetCheckbox(nullptr), speedSpinCtrl(nullptr), worldNumber(
				nullptr), buttonPanel(nullptr) {
	initialise();
}
/**
 *
...
wxPanel* MainFrameWindow::initialiseLhsPanel() {
	...
	sizer->Add(robotWorldCanvas = new View::RobotWorldCanvas(lhsPanel),
			wxGBPosition(1, 1), wxGBSpan(1, 1), wxSHRINK);
...
wxPanel* MainFrameWindow::initialiseRhsPanel() {
	rhsPanel = new wxPanel(splitterWindow, wxID_ANY, wxDefaultPosition,
			wxDefaultSize, wxBORDER);
	...
	sizer->Add(logPanel = initialiseLogPanel(), wxGBPosition(2, 1),
			wxGBSpan(1, 1), wxGROW);
	sizer->Add(configPanel = initialiseConfigPanel(), wxGBPosition(4, 1),
			wxGBSpan(1, 1), wxGROW);
	sizer->Add(buttonPanel = initialiseButtonPanel(), wxGBPosition(6, 1),
			wxGBSpan(1, 1), wxGROW);
	sizer->AddGrowableCol(1);
...
wxPanel* MainFrameWindow::initialiseLogPanel() {
	wxPanel *panel = new wxPanel(rhsPanel, wxID_ANY, wxDefaultPosition,
			wxDefaultSize, wxBORDER);
	...
	sizer->Add(
			logDestination =
					makeRadiobox(panel, choicesArray,
	...
	sizer->Add(
			logTextCtrl = new LogTextCtrl(panel, wxID_ANY,
					wxTE_MULTILINE | wxTE_DONTWRAP), wxGBPosition(2, 1),
			wxGBSpan(1, 1), wxGROW);
	sizer->AddGrowableRow(2);
	sizer->AddGrowableCol(1);
	...
	sizer->Add(
			makeButton(panel, "Clear log window",
...
wxPanel* MainFrameWindow::initialiseConfigPanel() {
	...
	sizer->Add(
			drawOpenSetCheckbox = Application::makeCheckbox(panel,
					"Draw OpenSet", [this](wxCommandEvent &event) {
						this->OnDrawOpenSet(event);
					}),
			wxGBPosition(1, 1), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTRE);

	/////// Speed
	sizer->Add(new wxStaticText(panel, wxID_ANY, "Speed"), wxGBPosition(2, 1),
			wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
	sizer->Add(speedSpinCtrl = new wxSpinCtrl(panel, wxID_ANY),
			wxGBPosition(2, 2), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
	speedSpinCtrl->SetValue(static_cast<int>(10));
	speedSpinCtrl->Bind(wxEVT_COMMAND_SPINCTRL_UPDATED,
			[this](wxCommandEvent &event) {
				this->OnSpeedSpinCtrlUpdate(event);
			});

	std::array<std::string, 8> choicesArray { "1 Situation 1.1",
			"2 Situation 1.2", "3 Situation 2.1", "4 Situation 2.2",
			"5 Situation 3.1", "6 Situation 3.2", "7 Situation 4.1",
			"8 Situation 4.2" };
	...
	sizer->Add(
			worldNumber =
					makeRadiobox(panel, choicesArray,
							...
							"World number", wxRA_SPECIFY_ROWS),
			wxGBPosition(3, 1), wxGBSpan(1, 1), wxSHRINK | wxALIGN_CENTER);
...
void MainFrameWindow::OnQuit(wxCommandEvent&UNUSEDPARAM(anEvent)) {
	Base::Trace::setTraceFunction(
			std::make_unique<Base::StdOutTraceFunction>());
	Close(true);
}
/**
 *
 ...
 *
 */
void MainFrameWindow::OnWorld7(wxCommandEvent &anEvent) {
	TRACE_DEVELOP(anEvent.GetString().ToStdString());
 ...
 *
 */
void MainFrameWindow::OnWorld8(wxCommandEvent &anEvent) {
	TRACE_DEVELOP(anEvent.GetString().ToStdString());
